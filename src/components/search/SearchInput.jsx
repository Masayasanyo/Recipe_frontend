...
const Input = ({ user, setMyList }) => {
  ...
  const handleKeyDown = async (event) => {
    if (event.key === "Enter") {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/recipe/mylist/search`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            }, 
            body: JSON.stringify({"account_id": user.id, "keyword": searchData, "option": searchOption}), 
        });
        if (response.ok) {
      ...
      <div className="group">
        ...
        <input 
         ...
         />
          ...
          <select onChange={(e) => optionChange(e.target.value)} className='search-options' name="search-options" id="search-option">
